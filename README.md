...
以这个门店管理首页为例，你可以看到我并没有导入一个`scss`文件，但是我使用了很多的`className`，这些就是用的全局样式，这全局样式都是[duxapp模块提供的](https://github.com)
      ...
      <ScrollView>
        ...
        <TipNav title='货品统计' url='arecaShop/storeManage/mallTotal'>
          <Row className='items-center' grow>
            ...
            <Text type='secondary' size={2}>{dayjs().format('MM/DD HH:mm')} 更新Text>
          Row>
        TipNav>
        <Card margin disableMarginBottom className='gap-2'>
          ...
          <Row justify='between' className='gap-2'>
            ...
            Column>
          Row>
          <Text size={22} type='secondary'>*销量及剩余仅供参考：记录消费者扫码数量，可能与实际结果有出入Text>
        Card>
        <TipNav title='配送记录'
          right={<ModalForm
            title='月份'
...
这些组件都是[duxui UI库](https://github.com)提供的
...
const ListPrice = ({ item, children }) => {
  if (item.type === 'point') {
    return <Text bold size={4} type='danger'>{item.sell_point}积分Text>
  }
  return children
}
...
开发文档：[http://duxapp.cn/](https://github.com):[蓝猫机场加速器](https://dahelaoshi.com)


GitHub：[https://github.com/duxapp](https://github.com)
